...
namespace Hones.Remit.Api.MassTransit.Events.OrderCollected;

public class OrderCollectedSenderEmailNotifier : IConsumer<OrderCollected>
{
    private readonly OrdersDbContext _dbContext;
    private readonly IEmailService _emailService;

    public OrderCollectedSenderEmailNotifier(OrdersDbContext dbContext, IEmailService emailService)
    {
    ...
    public async Task Consume(ConsumeContext<OrderCollected> context)
    {
        ...
        await _emailService.SendEmailAsync(order.SenderEmail, $"Order collected - {orderReference}",
